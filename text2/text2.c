...
/*一家超市出售鸡蛋包。一包6个鸡蛋售价S日元，一包8个鸡蛋售价M日元，一包12个鸡蛋售价L日元。当你可以购买任意数量的每种包装时，找出购买至少N个鸡蛋所需的最少金额。
约束条件：1 <= N <= 100,1 <= S,M,L <= 10^4，输入的都是整数。*/
//int main()
//{
//	int N, S, M, L;
//
...
/*给定一个又小写英文字母组成的字符串S，它又多少个非空子串？（子串是连续的子序列。例如，xxx是yxxxy的子串，而不是xxyxx的子串。约束条件：S的长度为1到100（包括两端）。*/
int main()
{
	char S[101];
	scanf("%d", &S);
